...
resource "google_container_cluster" "primary" {
  ...
  maintenance_policy {
    dynamic "recurring_window" {
      ...
      content {
        start_time = coalesce(
          try(
            local.maintenance_window_overrides["${var.project_id}-${local.cluster_name}"]["maintenance_start_time"],
            null
          ),
        ...
        end_time = coalesce(
          try(
            local.maintenance_window_overrides["${var.project_id}-${local.cluster_name}"]["maintenance_end_time"],
            null
          ),
        ...
        recurrence = coalesce(
          try(
            local.maintenance_window_overrides["${var.project_id}-${local.cluster_name}"]["maintenance_recurrence"],
            null
          ),
  ...
  provisioner "local-exec" {
    ...
    command     = "./post-cluster-create.sh"
  }
}
